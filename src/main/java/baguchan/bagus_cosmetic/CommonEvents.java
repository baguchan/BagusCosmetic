...
public class CommonEvents {
    @SubscribeEvent
    public static void spawnWithCosmetic(LivingSpawnEvent.SpecialSpawn event) {
    ...
    private static void equipAccessory(Mob mob, String identifier, ItemStack stack, String s) {
        CuriosApi.getCuriosHelper().getEquippedCurios(mob).ifPresent((handler) -> {
            boolean empty = true;
            for (SlotResult slotResult : CuriosApi.getCuriosHelper().findCurios(mob, identifier)) {
